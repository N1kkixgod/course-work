	...
	void Draw()
	{
		...
		for (int i = 0; i < numberOfLanes; i++)
		{
			for (int j = 0; j < width; j++)
			{
				...
				if (map[i]->CheckPos(j) && map[i]->CheckDir() && i != 0 && i != numberOfLanes - 1 && j != width - 1)
					cout << ">";
				else if (map[i]->CheckPos(j) && !(map[i]->CheckDir())&& i != 0 && i != numberOfLanes - 1 && j != width - 1)
					cout << "<";
